...
- **Cloud Transition**: Leverage existing Puppet data and automation to ease transitions to Kubernetes.
...
The following parameters in the `kubecm::deploy` plan specify key aspects of a deployment:
...
- **chart\_source**: The location of the Helm chart from a repo, a local directory, or an OCI registry reference. This tells Helm where to retrieve the chart. It can also be undefined to deploy without a chart, using only resources from Hiera.
- **repo_url**: If using a repo reference in `chart_source` (e.g. `repo_name/chart_name`), the HTTP location of the Helm repository.
...
Configuration is managed using Hiera's hierarchical structure. Create a `hiera.yaml` in the project directory:
...
These key names can be set in `data/common.yaml` to override the defaults as shown below:
...
This YAML plan defines a simple process to deploy the Vaultwarden application using KubeCM's `kubecm::deploy` plan.
...
This plan introduces the `parameters` block, which requires the user to specify the site (either `shop` or `blog`) to be deployed. This is useful when you want to reuse the same plan logic for multiple deployments.
...
The `render_to` parameter allows you to render the final output of the Helm or Kustomize configuration to a local file. This is useful for verifying the rendered Kubernetes manifests before deploying them.
...
After the command runs, the rendered configuration will be saved in `output.yaml` for review.
...
By default, the build directory is named `build` and is created in the current working directory.
